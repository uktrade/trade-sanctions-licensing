...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        stale_licences = Licence.objects.filter(created_at__lt=delete_threshold)
        for licence_object in stale_licences:
            licence_object.delete()
            self.stdout.write(self.style.SUCCESS(f"Successfully deleted Licence application {licence_object.id}"))
        ...
        email_licences = Licence.objects.filter(created_at__date=email_threshold.date())
